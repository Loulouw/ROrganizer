...
use crate::triggers::{key_to_vk, MouseBtn, Trigger, WheelDir};

pub fn draw(ctx: &egui::Context, app: &mut App) {
    let Some(slot) = app.capture_state().cloned() else {
        return;
    };
    let theme = app.theme();

    // 1) Esc clears any existing binding for this slot, then closes the modal.
    if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
        app.clear_binding(&slot);
        app.cancel_capture();
        return;
    }
    ...
    if let Some(trigger) = captured {
        app.commit_binding(slot.clone(), trigger);
        return;
    }
    ...
    egui::Area::new(egui::Id::new("capture_overlay"))
        ...
        .show(ctx, |ui| {
            // Backdrop covers the whole client area.
            ui.painter()
                    ...
                    Frame::none()
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.vertical_centered(|ui| {
                                ui.label(
                                    egui::RichText::new(
                                        t!("binding.modal_title").to_uppercase(),
                                    )
                                    .size(11.0)
    ...
    for ev in events {
        match ev {
            ...
            egui::Event::PointerButton {
                ...
                _ => {} // Primary / Secondary intentionally not capturable.
            },
